...
def venues():
    venue_city_groupings = db.session.query(Venue.city, Venue.state).group_by(Venue.city,
                                                                              Venue.state).all()
    venues_data = []
    ...
    if "search_term" in form.keys():
        ...
        for result in results:
            num_upcoming_shows = db.session.query(Show).join(Venue,
                ...
                db.cast(Show.start_time, db.Date) >= date.today()).count()
            data_item = {"id": result.id, "name": result.name,
...
def show_venue(venue_id):
    ...
    past_shows = db.session.query(Show.artist_id, Artist.name.label('artist_name'),
                                  ...
                                  Show.start_time).join(Artist, Show.artist_id == Artist.id).filter(
    ...
    data = {"id": selected_venue.id, "name": selected_venue.name, "genres": selected_venue.genres,
...
def show_artist(artist_id):
    ...
    past_shows = db.session.query(Show.venue_id, Venue.name.label('venue_name'),
                                  Venue.image_link.label('venue_image_link'), Show.start_time).join(
        Venue, Show.venue_id == Venue.id).filter(Show.artist_id == artist_id,
...
def edit_artist(artist_id):
    ...
    artist = db.session.query(Artist.id, Artist.name, Artist.genres, Artist.city, Artist.state,
                              Artist.phone, Artist.website_link, Artist.facebook_link,
                              Artist.seeking_venue, Artist.seeking_description,
                              Artist.image_link).filter(Artist.id == artist_id).first()
...
def edit_venue(venue_id):
    ...
    venue = db.session.query(Venue.id, Venue.name, Venue.genres, Venue.city, Venue.state,
                             Venue.address, Venue.phone, Venue.website_link, Venue.facebook_link,
                             Venue.seeking_talent, Venue.seeking_description,
                             Venue.image_link).filter(Venue.id == venue_id).first()
