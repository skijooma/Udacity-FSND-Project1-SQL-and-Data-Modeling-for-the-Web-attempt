...
This is also an attempt to the Udacity Fullstack Nanodegree - Project 1. The goal was to build out the data models to power the API endpoints for the Fyyur site by connecting to a PostgreSQL database for storing, querying, and creating information about artists and venues on Fyyur.

## Overview

The major task involved adding real data! Models were to be added, and a database to allow full CRUD interactions, in addition to existing Views and Controllers. The site should now be able to at least be capable of doing the following, if not more, using a PostgreSQL database:
...
```
> **Note** - If a specific version of a package is not mentioned, then the default latest stable package can be installed. 
...
## Main Files: Project Structure
  ...
  ├── error.log
  ├── forms.py *** Your forms
  ├── Genre.py *** Genre enum class for constraining form select field options
  ├── LICENSE.txt
  ├── migrations *** Directory for database migration scripts
  ├── package.json *** Javascript package dependencies listed in here.
  ├── README.md
  ├── requirements.txt *** The dependencies we need to install with "pip3 install -r requirements.txt"
  ├── State *** State enum class for constraining form select field options
  ├── static
  │   ├── css 
  │   ├── font
  │   ├── ico
